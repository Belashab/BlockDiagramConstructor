...
import DiagramCanvas$1 from './DiagramCanvas/DiagramCanvas.js';
...
let Diagram = function Diagram(props) {
  ...
  let onNodesChange = function onNodesChange(nextNodes) {
    if (onChange) {
      onChange({
        nodes: nextNodes
      });
  ...
  let onPortRegister = function onPortRegister(portId, portEl) {
    portRefs[portId] = portEl;
  };

  let onNodeRegister = function onNodeRegister(nodeId, nodeEl) {
    nodeRefs[nodeId] = nodeEl;
  };

  let onNodeRemove = useCallback(function (nodeId, inputsPorts, outputsPorts) {
  ...
  let onDragNewSegment = useCallback(function (portId, from, to, alignment) {
    setSegment({
  ...
  let onSegmentFail = useCallback(function () {
    setSegment(undefined);
  }, []);

  let onSegmentConnect = function onSegmentConnect(input, output) {
    let nextLinks = [].concat(_toConsumableArray(schema.links || []), [{
      input: input,
      output: output
    }]);
  ...
  let onLinkDelete = function onLinkDelete(nextLinks) {
    if (onChange) {
      onChange({
        links: nextLinks
      });
  ...
  return React.createElement(DiagramCanvas$1, _extends({
    portRefs: portRefs,
    nodeRefs: nodeRefs
...
let Diagram$1 = React.memo(Diagram);
