...
let _rollupPluginBabelHelpers = require('../../_virtual/_rollupPluginBabelHelpers.js');
let PropTypes = require('prop-types');
let classNames = require('classnames');
let Types = require('../../shared/Types.js');
let getDiagramNodeStyle = require('./getDiagramNodeStyle.js');
let useContextRegistration = require('../../shared/internal_hooks/useContextRegistration.js');
let useDrag = require('../../shared/internal_hooks/useDrag.js');
let portGenerator = require('./portGenerator.js');
let useNodeUnregistration = require('../../shared/internal_hooks/useNodeUnregistration.js');

function _interopDefaultLegacy (e) { return e && typeof e === 'object' && 'default' in e ? e : { 'default': e }; }

let React__default = /*#__PURE__*/_interopDefaultLegacy(React);
let PropTypes__default = /*#__PURE__*/_interopDefaultLegacy(PropTypes);
let classNames__default = /*#__PURE__*/_interopDefaultLegacy(classNames);

let DiagramNode = function DiagramNode(props) {
  let id = props.id,
  ...
  let registerPort = useContextRegistration.usePortRegistration(inputs, outputs, onPortRegister);

  let _useDrag = useDrag['default']({
    throttleBy: 14
  }),
      ref = _useDrag.ref,
  ...
  let dragStartPoint = React.useRef(coordinates);
  ...
  if (!disableDrag) {
    ...
    onDrag(function (event, info) {
      if (onPositionChange) {
        ...
        let nextCoords = [dragStartPoint.current[0] - info.offset[0], dragStartPoint.current[1] - info.offset[1]];
  ...
  let classList = React.useMemo(function () {
  ...
  let InputPorts = inputs.map(portGenerator['default'](options, 'input'));
  let OutputPorts = outputs.map(portGenerator['default'](options, 'output'));
  let customRenderProps = {
    id: id,
    render: render,
    content: content,
    type: type,
...
let DiagramNode$1 = React__default['default'].memo(DiagramNode);
